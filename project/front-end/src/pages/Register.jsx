...
const Register = () => {
    ...
    const handleSubmit=(e)=>{
        e.preventDefault()
        if(username.length<3 || (/^[\w-\.]+@([\w-]+\.)+[\w-]{2,4}$/g.test(email))==false || password.length<3 ){
            setError(true)
        }
        if(username.length>=3&&(/^[\w-\.]+@([\w-]+\.)+[\w-]{2,4}$/g.test(email))==true&&password.length>=3){
            setError(false)
            handleRegister()
        }
    }
    ...
    return (
        ...
        <h1 className="text-lg md:text-xl font-extrabold"><Link to="/">Auto Forum</Link></h1>
        <h3><Link to="/login">Login</Link></h3>
        </div>
        <div className="w-full flex justify-center items-center h-[90vh] ">
            <div className="flex flex-col justify-center items-center space-y-4 w-[80%] md:w-[25%]">
                <form onSubmit={handleSubmit} className="flex flex-col justify-center items-center space-y-4 w-[100%] md:w-[100%]">
                ...
                <label className="text-red-600">Username must be at least 3 characters!</label>:""}
                ...
                {error&&((/^[\w-\.]+@([\w-]+\.)+[\w-]{2,4}$/g.test(email))==false)?
                ...
                <label className="text-red-600">Password must be at least 3 characters!</label>:""}
                ...
                {error==true && <h3 className="text-red-500 text-sm ">Correct data if neccesary!</h3> }
